...
Every Incident RCA MUST be based on a specific, verifiable real-world event (e.g., CSB reports, OSHA investigations, IEEE case studies, documented whitepapers). The AI must hold this real-world incident in context and communicate the original source to the user. Do not invent purely hypothetical or composite incidents without the user's explicit approval. 

*(Note: While the incident must be real, the published blog post must still follow the project's Anonymization Standard, removing specific facility, owner, and contractor names).*

## 10-Part Narrative Format
...
- **Format:** Embedded PNG via click-to-expand markdown pattern, produced by Claude using `fmea_renderer.py`. Just insert the placeholder text: `*(Note: FMEA rendering to be completed by Claude editorial agent prior to publication).*` along with the image tag.
...
### 8. Resources
- **Format:** A short paragraph + link to a paired field checklist PDF (built by Claude using `lead_magnet_builder.py` + `lead_magnet_template.html`). Insert the placeholder `*[Lead magnet CTA — Claude]*`.
...
### 10. Bottom Line
- **Format:** A powerful, one-sentence systemic lesson that serves as the final takeaway of the post.
