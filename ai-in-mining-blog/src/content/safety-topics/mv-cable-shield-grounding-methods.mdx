...
LINKEDIN POST DRAFT:
Hook: Grounding both ends of a medium-voltage cable shield seems like the safest choice—but it could be secretly cooking your cable insulation.
Setup: MV power cables require a metallic shield to equalize electric fields. However, the AC load current induces a voltage along this shield like a transformer secondary.
Core Failure: Grounding both ends eliminates touch voltage but creates a closed loop, generating continuous circulating currents that heat up the cable. Grounding one end eliminates current but leaves a dangerous shock hazard at the open end.
Takeaway: You must calculate the induced sheath voltage. If it exceeds 50V under load, you need a Sheath Voltage Limiter (SVL) or trefoil layout to cancel the magnetic fields.
CTA: Which grounding philosophy does your facility use for MV cables: single-point or dual-point?
Hashtags: #MediumVoltage #SubstationDesign #CableShielding #GroundingAndBonding #ElectricalSafety #PowerSystems
*/}
